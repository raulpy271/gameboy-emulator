...
TEST(ArithmeticInstructions, OR_A_D_instruction) {
  gameboy::Console game;
  game.initialize_registers();
  game.mem.SetInAddr(0x100, OR_A_D, true);
  game.cpu.reg.A = 0b00000100;
  game.cpu.reg.D = 0b01000101;
  game.cpu.reg.F = 0x0;
...
TEST(ArithmeticInstructions, ADD_A_D_instruction) {
  ...
  EXPECT_EQ(game.cpu.reg.A, 0x20 + 0x10);
  EXPECT_EQ(game.cpu.reg.PC, 0x101);
  EXPECT_EQ(utils::zero_flag(&game.cpu.reg.F), false);
}
...
TEST(ArithmeticInstructions, DEC_H_instruction) {
  gameboy::Console game;
  game.initialize_registers();
  game.mem.SetInAddr(0x100, DEC_H, true);
  game.cpu.reg.H = 0x02;
  game.cpu.reg.F = 0x0;
  game.cpu.execute_intruction(&game.mem);
